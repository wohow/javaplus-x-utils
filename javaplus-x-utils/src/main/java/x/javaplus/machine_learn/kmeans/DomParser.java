package x.javaplus.machine_learn.kmeans;
...
public class DomParser {
	...
	public ArrayList<General> prepare() {
		...
		Document doc = null;
		try {
			doc = builder.parse(new File("G:/猎豹浏览器下载中心/general.xml"));
		...
		for (int i = 1; i < nodeList.getLength(); i++) {
			...
			generals.add(new General(Tool.xingji(attList.item(1)
					...
					Integer.parseInt(attList.item(5).getTextContent()), Integer
							.parseInt(attList.item(7).getTextContent()),
					Integer.parseInt(attList.item(9).getTextContent()), Integer
							.parseInt(attList.item(11).getTextContent()), Tool
							.change(attList.item(13).getTextContent()), Tool
							.change(attList.item(15).getTextContent()), Tool
							.change(attList.item(17).getTextContent()), Tool
							.change(attList.item(19).getTextContent()), Tool
							.change(attList.item(21).getTextContent()), Integer
							.parseInt(attList.item(23).getTextContent()),
